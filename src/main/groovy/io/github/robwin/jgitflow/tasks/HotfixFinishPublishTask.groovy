...
class HotfixFinishPublishTask extends AbstractCommandTask  {
    ...
    void finish(){
		...
		if(hotfixName=='hostfixFinishPublish') {
	        def command = flow.hotfixFinish('hostfixFinish')
			def command1 = flow.hotfixPublish('hostfixPublish')
			...
			setCommandPrefixAndSuffix(command1)
